...
// mohsen
    Provider.of<ProductDetailsProvider>(Get.context!, listen: false)
        .removePrevReview();
    Provider.of<ProductDetailsProvider>(Get.context!, listen: false)
        .initProduct(widget.orderModel!.customer!.id, Get.context!);
  ...
  Widget build(BuildContext c) {
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
        ...
        child: Consumer2<OrderProvider, ProductDetailsProvider>(
            builder: (context, order, review, child) {
          double itemsPrice = 0;
          double discount = 0;
          double? eeDiscount = 0;
    ...
    Future.delayed(const Duration(seconds: 1), () {
                                ...
                                orderModel: widget.orderModel,
                                averageReview:
                                    " ${Provider.of<OrderProvider>(context, listen: false).orderModel!.orders![widget.index!].customer!.rating?.avg ?? 0} ")
                            : const SizedBox(),
                        const SizedBox(
                            height: Dimensions.paddingSizeExtraSmall),
                        widget.orderModel!.deliveryMan != null
                            ? Padding(
                                ...
                                child: DeliveryManContactInformation(
                                    orderModel: widget.orderModel,
                                    orderType: widget.orderType,
                                    onlyDigital: _onlyDigital),
                              )
